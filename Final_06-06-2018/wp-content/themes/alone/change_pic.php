...
function changeProfilePic() {
    //print_r($_FILES['profile-pic']);die();
$post = isset($_POST) ? $_POST: array();
$max_width = "500";
...
if(strlen($name)) {
    ...
    if($size < (1024*1024)) { $actual_image_name = 'avatar' .'_'.$userId .'.'.$ext;
    $filePath = $path .'/'.$actual_image_name;
    $tmp = $_FILES['profile-pic']['tmp_name'];
    
    
    if(move_uploaded_file($tmp, $filePath)) { $width = getWidth($filePath);
    $height = getHeight($filePath); //Scale the image if it is greater than the width set above if ($width > $max_width){
